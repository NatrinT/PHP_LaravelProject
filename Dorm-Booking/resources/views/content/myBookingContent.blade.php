...
<div class="page-wrap py-4">
    ...
    <div class="d-md-none">
        @forelse ($bookings as $b)
            @php
                $room = $b->room; // ใช้ relation ตรง ๆ
                $img = $room?->image
                        ? asset('storage/'.$room->image)
                        : (\Illuminate\Support\Facades\Storage::disk('public')->exists(
                                'uploads/branch/' . \Illuminate\Support\Str::slug($room?->branch ?? '') . '.jpg'
                          )
            ...
            <div class="card book-card mb-3 overflow-hidden">
                <img src="{{ $img }}" class="roomimg" alt="">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-start mb-1">
                        <div>
                            <div class="fw-bold">Room {{ $room?->room_no }}</div>
                            <div class="text-muted small">
                                {{ ucwords(strtolower($room?->branch ?? '')) }}
                                • ชั้น {{ $room?->floor }}
                    ...
                    <div class="row g-2 small text-muted my-2">
                        <div class="col-6">เริ่มสัญญา:
                            <b class="text-dark">{{ optional(\Illuminate\Support\Carbon::parse($b->start_date))->format('Y-m-d') }}</b>
                        </div>
                        <div class="col-6">หมดสัญญา:
                            <b class="text-dark">{{ optional(\Illuminate\Support\Carbon::parse($b->end_date))->format('Y-m-d') }}</b>
                        </div>
                        <div class="col-6">ค่าเช่า/เดือน:
                            <b class="text-dark">THB {{ number_format($room?->monthly_rent ?? 0, 2) }}</b>
                        </div>
                        <div class="col-6">ยอดล่าสุด:
                            <b class="text-dark">THB {{ number_format($b->last_bill_amount ?? ($room?->monthly_rent ?? 0), 2) }}</b>
                        </div>
                    </div>

                    <div class="d-flex justify-content-between align-items-center">
                        <span class="{{ $payBadge }} small">
                            @switch($pay)
        ...
        @empty
            <div class="alert alert-info">ยังไม่มีการจอง/สัญญา</div>
        @endforelse
