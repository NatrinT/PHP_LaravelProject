    ...
    <form action="/invoice" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group row mb-2">
            ...
            <div class="col-sm-6">
                ...
                @error('lease_id') <div class="text-danger">{{ $message }}</div> @enderror
            </div>
        </div>
        ...
        <div class="form-group row mb-2">
            ...
            <div class="col-sm-6">
                <input type="text" class="form-control" name="billing_period"
                       value="{{ old('billing_period') }}" required>
                @error('billing_period') <div class="text-danger">{{ $message }}</div> @enderror
            </div>
        </div>
        ...
        <div class="form-group row mb-2">
            ...
            <div class="col-sm-4">
                <input readonly type="number" class="form-control" step="0.01" min="0"
                       name="amount_rent" id="amount_rent"
                       value="{{ old('amount_rent') }}" required>
                @error('amount_rent') <div class="text-danger">{{ $message }}</div> @enderror
            </div>

            <label class="col-sm-2">Utilities</label>
            <div class="col-sm-4">
                <input type="number" class="form-control" step="0.01" min="0"
                       name="amount_utilities" id="amount_utilities"
                       value="{{ old('amount_utilities') }}">
                @error('amount_utilities') <div class="text-danger">{{ $message }}</div> @enderror
            </div>
        </div>
        ...
        <div class="form-group row mb-2">
            ...
            <div class="col-sm-4">
                <input type="number" class="form-control" step="0.01" min="0"
                       name="amount_other" id="amount_other"
                       value="{{ old('amount_other') }}">
                @error('amount_other') <div class="text-danger">{{ $message }}</div> @enderror
            </div>
        ...
        <div class="form-group row mb-2">
            ...
            <div class="col-sm-6">
                ...
                @error('receipt_file') <div class="text-danger">{{ $message }}</div> @enderror
            </div>
        </div>
...
(function () {
    ...
    function setRentFromLease() {
        ...
        const rent = opt ? opt.getAttribute('data-rent') : null;
    ...
    // preload ครั้งแรก: ถ้าเลือก lease แล้วแต่ยังไม่ได้ใส่ rent ให้ auto ใส่
